...
public class CmdGUI extends GUI {
	...
	private void createTrackingGroupPrompts() {
		...
		//database
		boolean usesDatabase = getInput("Should files belonging to this group be tracked within the file system? [y/n]: ")
		...
		Configuration.addTrackingGroup(new TrackingGroup(groupName, usesDatabase, algorithm, key));
	...
	private Key chooseKeyPrompts(ProductMode mode, String groupName) {
		...
		//key name
		String keyName = getInput("Enter a name for your password or key file (like a hint): ");
		...
		{
			if (getInput("Would you like to use a key file? [y/n]: ").toLowerCase().equals("y"))
			{
				//key file
				File keyFile = null;
				String input = getInput("Enter key file location (or return to have the system prompt for it when needed): ");
				if (input.length() > 0)
				{
