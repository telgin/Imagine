	...
	private final void fourEnforcement(int val) throws ProductIOException
	{
		...
		for (int i=0; i<4; ++i)
		{
			...
			if (pv[4] == 3044 && pv[5] ==1690)
			{
				...
				int r1 = ByteConversion.byteToInt(ImageUtil.getRed(img.getRGB(3044, 1690)));
				int g1 = ByteConversion.byteToInt(ImageUtil.getGreen(img.getRGB(3044, 1690)));
				int b1 = ByteConversion.byteToInt(ImageUtil.getBlue(img.getRGB(3044, 1690)));
				System.out.println("Independantly evaluated: " + r1 + ", " + g1 + ", " + b1);
			}
			
			int bytes = 3642821;
			if ((byteCount > bytes - 5 && byteCount < bytes + 5) || c0 != toSet || fourVals[i] < 0 || fourVals[i] > 3)
			{
				...
				System.out.println(pv[0] + ", " + pv[1] + ", " + pv[2] + ", " + pv[3] + ", " + pv[4] + ", " + pv[5]);
//				System.out.println(Pattern.validIndex(pattern, pv[0], pv[1], img.getWidth(), img.getHeight()));
//				System.out.println(Pattern.validIndex(pattern, pv[2], pv[3], img.getWidth(), img.getHeight()));
//				System.out.println(Pattern.validIndex(pattern, pv[4], pv[5], img.getWidth(), img.getHeight()));
			
			}
		}
		
		
			boolean truth = val == (((fourVals[0] * 4) + fourVals[1]) * 16) + ((fourVals[2] * 4) + fourVals[3]);
			if (!truth)
				System.out.println ("No truth..." + val + ", " + (((fourVals[0] * 4) + fourVals[1]) * 16) + ((fourVals[2] * 4) + fourVals[3]));
	...
	private void setColor(int x, int y, byte data)
	{
		...
		if (x == 3044 && y ==1690)
		{
			System.out.println("Setting color " + colorIndex + " to " + ByteConversion.byteToInt(data));
			System.out.println();
		}
	...
	public int write(byte[] bytes, int offset, int length)
	{
		...
		{
			if (pv[0] == 3044 && pv[1] == 1690)
				System.out.println("Happened by write array: " + offset + ", " + length + ", " + x + ", " + bytes.length);
		...
		try {
			...
			for (int x=0; x<img2.getWidth(); ++x)
			{
				for (int y=0; y<img2.getHeight(); ++y)
				{
					...
					int g1 = ByteConversion.byteToInt(ImageUtil.getGreen(img.getRGB(x, y)));
					int b1 = ByteConversion.byteToInt(ImageUtil.getBlue(img.getRGB(x, y)));
					int r2 = ByteConversion.byteToInt(ImageUtil.getRed(img2.getRGB(x, y)));
					int g2 = ByteConversion.byteToInt(ImageUtil.getGreen(img2.getRGB(x, y)));
					int b2 = ByteConversion.byteToInt(ImageUtil.getBlue(img2.getRGB(x, y)));
